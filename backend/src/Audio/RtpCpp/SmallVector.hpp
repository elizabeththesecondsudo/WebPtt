...
protected:
  ...
  static void uninitialized_copy(It1 I, It1 E, It2 Dest) {
    if constexpr (std::is_pointer_v<It1> && std::is_pointer_v<It2> &&
      ...
      if (I != E)
        std::memcpy(reinterpret_cast<void *>(Dest), I, static_cast<size_t>(E - I) * sizeof(T));
    } else {
...
SmallVector<ValueTypeFromRangeType<R>, Size> to_vector(R &&Range) {
  return SmallVector<ValueTypeFromRangeType<R>, Size>(std::begin(Range),
                                                      std::end(Range));
}
template <typename R>
SmallVector<ValueTypeFromRangeType<R>> to_vector(R &&Range) {
  return SmallVector<ValueTypeFromRangeType<R>>(std::begin(Range),
                                                std::end(Range));
}
